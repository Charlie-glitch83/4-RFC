# Supplement S1: Raw Wolfram Reproducibility Trace — Final Revised W2 BBN/Li7 Closeout Metadata

## Final Revision Notice: W2 BBN/Li7 Scalar-to-Triadic-Weight Correction

**Revision status:** Final revised reproducibility metadata with W2 BBN/Li7 internal closeout.  
**Revision purpose:** Preserve the original RFC reproducibility metadata while adding the completed W2 BBN/Li7 correction ledger. This revision records that the earlier lithium-7 wall was diagnosed as a projection-operator problem: the old BBN lane used a collapsed triad scalar where the corrected lane uses channel-specific QV/CIF/RFL triadic weights.

### High-Level Status Change

```text
Old W2 BBN status: Li7 wall / unresolved under scalar proxy.
Revised W2 BBN internal status: PASS under triadic channel-weight projection.
Theorem addendum status: READY, with public-code boundary.
External public-code status: PENDING.
```

The corrected W2 BBN lane changes only the downstream projection operator. It does **not** retune the frozen Module G packet and does **not** use MCMC, NUTS, empirical target assimilation, or parameter search.

```text
Correction = Replace collapsed triad scalar with triadic channel weights.
FailureDiagnosed = Old scalar BBN lane erased CIF/QV/RFL channel structure and left Li7 overproduced.
MechanismRecovered = QV-dominant branch-selective suppression of the Be7/Li7 pathway.
FrozenModuleGPacketUsed = True
TriadScalarUsed = False
TriadicWeightsUsed = True
EmpiricalTargetsUsedInPrediction = False
...
This revision closes the **internal theorem/proxy BBN lane** only. It does not claim completed external BBN validation and does not claim a completed external solution to the cosmological lithium problem. External validation remains pending until the triadic-weight operator is implemented in a public BBN reaction-network code such as AlterBBN, PArthENoPE, PRIMAT, PRyMordial, LINX, or an equivalent reproducible public pipeline.

---

## MODULE-W2-BBN-FINAL-CLOSEOUT-AUDIT

```text
MODULE-W2-BBN-FINAL-CLOSEOUT-AUDIT

Purpose = Lock the revised BBN/Li7 correction into a theorem-ready internal closeout ledger.
...
RealWorldPassedSpeciesCount = 4.00000000000
RealWorldTotalSpeciesCount = 4.00000000000

CheckLedger:
ScalarLaneFailedAsExpected = True
TriadicLi7Recovered = True
Li7SuppressionMatchesProblemScale = True
RobustnessStrictModePass = True
RobustnessBridgeModePass = True
LightElementsStable = True
Be7BranchSuppressed = True
ChannelSelectivityPass = True
QVMechanismPass = True
RealWorldObservationalProxyPass = True
NoRetuneConditionPreserved = True
FrozenPacketPreserved = True
PublicBBNCodeCompleted = False

InternalBBNProxyStatus = PASS
ExternalPublicCodeStatus = PENDING
ReadyForTheoremAddendum = YES_WITH_PUBLIC_CODE_BOUNDARY

TheoremAddendumStatus = READY

ClosedInternalClaim = RFC W2 BBN/Li7 scalar-lane failure is corrected by triadic channel weighting under the frozen no-retune packet. The revised lane passes proxy, robustness, channel-selectivity, ablation, and real-world-facing observational comparison audits.

ClaimBoundary = This closes the internal theorem/proxy BBN lane. It does not claim completed external BBN validation until implemented in a public BBN reaction-network code such as AlterBBN or PArthENoPE.
```

---

## Revised Module W Theorem Status

```text
Module W = Primordial Triadic Nucleosynthesis / BBN
Scope = D/H, He3/H, YpHe4, Be7/H, Li7/H
Internal status = PASS
Theorem addendum status = READY
External public-code status = PENDING
```

Module W should be added to the theorem as a **no-retune downstream projection audit**. Its role is not to claim external validation, but to record that the frozen CIF/QV/RFL packet survives projection into the primordial abundance proxy lane once the scalar correction is replaced by triadic channel weights.

### Theorem-Safe Insert Summary

```text
Module W closes the internal RFC BBN/Li7 proxy lane by replacing the failed scalar correction with a no-retune triadic-channel weighting operator. The revised lane passes proxy recovery, two-mode robustness, channel selectivity, component ablation, and real-world-facing abundance-band comparison. This does not constitute completed public BBN-network validation, which remains pending.
```

---

## Complete Prior Revised Metadata and Original Trace Preserved Below

The following section preserves the previous revised W2 Li7 metadata addendum and the original raw reproducibility metadata trace.

---

# Supplement S1: Raw Wolfram Reproducibility Trace — Revised W2 Li7 Triadic-Weight Metadata

## Revision Notice: W2 Li7 Correction Closeout

**Revision status:** Revised metadata file with W2 Li7 triadic-weight correction addendum.  
**Revision purpose:** Preserve the original reproducibility metadata while adding the corrected W2 BBN/Li7 lane developed after diagnosing that the earlier Li7 proxy failure came from using a collapsed triad scalar rather than channel-specific QV/CIF/RFL weights.

### Status Change

The earlier W2 ledger retained Li7 as an internal wall because the scalar BBN proxy left the Li7/H channel overproduced by approximately 3.125x. That result remains historically preserved in the original trace.

The revised W2 correction changes only the **projection operator**, not the frozen Module G packet:

```text
Old operator:     collapsed triad scalar
Revised operator: triadic channel weights over QV/CIF/RFL
```

The corrected W2 proxy lane now records:

```text
MODULE-W2-LI7-CORRECTION-CLOSEOUT = PASS
Correction = Replace collapsed triad scalar with triadic channel weights.
FailureDiagnosed = Old W2 Li7 failure arose from scalar compression of QV/CIF/RFL structure.
MechanismRecovered = QV-dominant branch-selective suppression of Be7/Li7.
NoRetunePreserved = True.
FrozenPacketPreserved = True.
PublicCodeBoundary = Still required before external Li7 solution claim.
```

### Claim Boundary

This revision **does not** claim a completed external Li7 solution. It upgrades the internal RFC W2 lane from:

```text
Old internal status: Li7 wall / unresolved under scalar proxy
New internal status: Triadic-weight proxy pass
External status: Public BBN-network validation pending
```

Full public BBN-network validation through AlterBBN, PArthENoPE, PRIMAT, PRyMordial, LINX, or equivalent public reaction-network implementation remains required before claiming external resolution of the cosmological lithium problem.

---

## Revised W2 Li7 Metadata Addendum

### Frozen Packet / No-Retune Conditions
...
The frozen Module G packet remains unchanged:
...
The corrected BBN/Li7 projection uses the triadic-weight packet inherited from Module R:

```text
StrictNoBridge:
QVWeight = 0.981311
CIFWeight = 0.00628018
RFLWeight = 0.0124092

InterpretiveDarkBridge:
QVWeight = 0.9848683151578609
CIFWeight = 0.0050846816270981214
RFLWeight = 0.010047003215041029
```

---

## MODULE-W2-LI7-TRIADIC-WEIGHT-PROXY-PASS

```text
MODULE-W2-LI7-TRIADIC-WEIGHT-PROXY-PASS

RunName = RFC-W2-BBN-Triadic-Weight-Revision
TriadScalarUsed = False
TriadicWeightsUsed = True
FrozenPacketUsed = True
EmpiricalTargetsUsedInPrediction = False

KernelGain = 1.54099
OldLi7OverObserved = 3.12534
TriadicLi7OverObserved = 0.999643
Li7SuppressionFactorVsOldScalar = 3.12645
He4Drift = 0.000390252
DPercentDrift = 0.801015

Interpretation = The old scalar triad lane leaves Li7 overproduced by about 3.125x, while the revised triadic-weight lane suppresses the Be7/Li7 branch to approximately the observational anchor without destabilizing D/H or He4.

ClaimBoundary = Proxy only; public BBN-network validation through AlterBBN or PArthENoPE remains required.
```

---

## MODULE-W2-LI7-TRIADIC-WEIGHT-ROBUSTNESS-AUDIT

```text
MODULE-W2-LI7-TRIADIC-WEIGHT-ROBUSTNESS-AUDIT

Purpose = Check whether the Li7 recovery survives both Module R triad-weight interpretations without retuning.
...
CIFWeight = 0.00628018
RFLWeight = 0.0124092
TriadicLi7OverObserved = 1.0034
Li7SuppressionFactorVsOldScalar = 3.11475
Be7SuppressionFactor = 2.88762
DPercentDrift = 0.807492
He3PercentDrift = 0.645427
He4Drift = 0.000391621
ModeStatus = PASS

Mode = InterpretiveDarkBridge
QVWeight = 0.984868
CIFWeight = 0.00508468
RFLWeight = 0.010047
TriadicLi7OverObserved = 0.999643
Li7SuppressionFactorVsOldScalar = 3.12645
Be7SuppressionFactor = 2.89767
...
PassRule = Li7 ratio between 0.80 and 1.20, D/H drift <= 2%, He3/H drift <= 2%, He4 absolute drift <= 0.002.

Interpretation = If both modes pass, the Li7 correction is not a one-mode artifact. The scalar lane failed because it collapsed the triad, while the triadic-weight lane preserves channel selectivity across QV/CIF/RFL.

ClaimBoundary = Internal proxy robustness audit only. Public BBN-network implementation remains required before claiming external Li7 resolution.
```

---

## MODULE-W2-LI7-TRIADIC-WEIGHT-CHANNEL-SELECTIVITY-AUDIT

```text
MODULE-W2-LI7-TRIADIC-WEIGHT-CHANNEL-SELECTIVITY-AUDIT

Purpose = Test whether the correction is localized to the Be7/Li7 branch rather than globally distorting the light-element bundle.
...
SpeciesBreakdown:
D/H | TriadicExponent = 0.00797824 | NewValue = 2.57043e-5 | PercentDrift = 0.801015 | SuppressionVsBaseline = 0.992054
He3/H | TriadicExponent = 0.00638221 | NewValue = 1.03659e-5 | PercentDrift = 0.640262 | SuppressionVsBaseline = 0.993638
YpHe4 | TriadicExponent = 0.00156431 | NewValue = 2.49668e-1 | PercentDrift = 0.156553 | SuppressionVsBaseline = 0.998437
Be7/H | TriadicExponent = -1.06391 | NewValue = 1.60474e-10 | PercentDrift = -65.4895 | SuppressionVsBaseline = 2.89767
Li7/H | TriadicExponent = -1.13979 | NewValue = 1.59943e-10 | PercentDrift = -68.0114 | SuppressionVsBaseline = 3.12612

PassRule = Li7 ratio between 0.80 and 1.20, Be7 suppression >= 2.50, and maximum D/H-He3/H-He4 drift <= 2%.
ModuleStatus = PASS

Interpretation = A pass means the triadic-weight correction is channel-selective: it suppresses the Be7/Li7 branch while leaving D/H, He3/H, and He4 comparatively stable.

ClaimBoundary = Internal proxy channel-selectivity audit only. Public BBN-network implementation remains required before external Li7-resolution claims.
```

---

## MODULE-W2-LI7-TRIADIC-WEIGHT-COMPONENT-ABLATION-AUDIT

```text
MODULE-W2-LI7-TRIADIC-WEIGHT-COMPONENT-ABLATION-AUDIT

Purpose = Test whether the Li7 correction is driven by the QV selection/compression channel rather than by CIF, RFL, or a generic scalar shift.
...
KernelGain = 1.5409877507963712
OldScalarLi7OverObserved = 3.125337740124628

Mode = FullTriad
QVWeight = 0.9848683151578609
CIFWeight = 0.0050846816270981214
RFLWeight = 0.010047003215041029
Li7OverObserved = 0.9996430364381471
Li7SuppressionVsOldScalar = 3.1264537701984065
Be7SuppressionFactor = 2.8976724462359997
DPercentDrift = 0.8010147034526405
He3PercentDrift = 0.6402621480304349
He4Drift = 0.0003902521015539395
MaxLightElementAbsDriftPercent = 0.8010147034526405
ModeStatus = PASS
...
CIFWeight = 0.
RFLWeight = 0.
Li7OverObserved = 0.9838257018273748
Li7SuppressionVsOldScalar = 3.1767189394621136
Be7SuppressionFactor = 2.9408287387503185
DPercentDrift = 0.773469817699346
He3PercentDrift = 0.6182987241885485
He4Drift = 0.0003844304697917522
MaxLightElementAbsDriftPercent = 0.773469817699346
ModeStatus = PASS
...
CIFWeight = 1.
RFLWeight = 0.
Li7OverObserved = 3.645644102185338
Li7SuppressionVsOldScalar = 0.8572799901809344
Be7SuppressionFactor = 0.8571873480811679
DPercentDrift = 4.73148809054309
He3PercentDrift = 3.1299600552878504
He4Drift = 0.0007694537989056394
MaxLightElementAbsDriftPercent = 4.73148809054309
ModeStatus = FAIL_OR_EXPECTED_FAIL

Mode = RFLOnly
QVWeight = 0.
CIFWeight = 0.
RFLWeight = 1.
Li7OverObserved = 2.4800683507140873
Li7SuppressionVsOldScalar = 1.2601820991041428
Be7SuppressionFactor = 1.2600459173232934
DPercentDrift = 1.5529221909876156
He3PercentDrift = 1.5529221909876156
He4Drift = 0.0007694537989056394
MaxLightElementAbsDriftPercent = 1.5529221909876156
ModeStatus = FAIL_OR_EXPECTED_FAIL

Mode = NoQV_Renormalized
QVWeight = 0.
CIFWeight = 0.33602878199909064
RFLWeight = 0.6639712180009093
Li7OverObserved = 2.8228307112946758
Li7SuppressionVsOldScalar = 1.107164424568418
Be7SuppressionFactor = 1.107044778667133
DPercentDrift = 2.610102031479089
He3PercentDrift = 2.0801435520743095
He4Drift = 0.0007694537989056394
MaxLightElementAbsDriftPercent = 2.610102031479089
ModeStatus = FAIL_OR_EXPECTED_FAIL

MechanismCheck = FullTriad should pass; QVOnly should remain near-pass or pass; CIFOnly/RFLOnly should fail; NoQV_Renormalized should fail.
MechanismStatus = PASS

Interpretation = A pass means the Li7 recovery is QV-dominant. CIF and RFL do not independently solve Li7; they refine the full triadic projection while QV supplies the branch-selective suppression.

ClaimBoundary = Internal proxy ablation audit only. Public BBN-network implementation remains required before external Li7-resolution claims.
```

---

## MODULE-W2-LI7-TRIADIC-WEIGHT-REAL-WORLD-OBSERVATIONAL-COMPARISON

```text
MODULE-W2-LI7-TRIADIC-WEIGHT-REAL-WORLD-OBSERVATIONAL-COMPARISON

Purpose = Compare the revised triadic-weight BBN proxy output against observational abundance bands for D/H, He3/H, Yp He4, and Li7/H.
...
OldScalarLi7OverObservedCenter = 3.125337740124628
TriadicLi7OverObservedCenter = 0.9996430364381471
Li7SuppressionFactorVsOldScalar = 3.1264537701984065

Species = D/H
Prediction = 0.000025704258749380422
ObservedCenter = 0.000025270000000000004
ObservedBandLow = 0.000024670000000000003
ObservedBandHigh = 0.000025870000000000005
PredictionOverObservedCenter = 1.017184754625264
BandStatus = PASS
BandZProxy = 0.7237645823006956

Species = He3/H
Prediction = 0.000010365947001247136
ObservedCenter = 0.00001
ObservedBandLow = 7.500000000000001e-6
ObservedBandHigh = 0.000013000000000000001
PredictionOverObservedCenter = 1.0365947001247136
BandStatus = PASS
BandZProxy = 0.13307163681714013

Species = YpHe4
Prediction = 0.24966825210155394
ObservedCenter = 0.245
ObservedBandLow = 0.239
ObservedBandHigh = 0.251
PredictionOverObservedCenter = 1.0190540902104244
BandStatus = PASS
BandZProxy = 0.7780420169256566

Species = Li7/H
Prediction = 1.5994288583010356e-10
ObservedCenter = 1.6000000000000002e-10
ObservedBandLow = 1.2e-10
ObservedBandHigh = 2.2000000000000002e-10
PredictionOverObservedCenter = 0.9996430364381471
BandStatus = PASS
BandZProxy = -0.0011422833979292065

PassedSpeciesCount = 4 of 4
ModuleStatus = PASS

Interpretation = This is the first real-world-facing observational comparison after the scalar-to-triadic-weight correction. A pass means the revised proxy sits inside the selected observational bands while preserving the no-retune condition.

ClaimBoundary = Observational proxy comparison only. Full public BBN-network validation through AlterBBN or PArthENoPE remains required before claiming an external Li7 solution.
```

---

## Revised W2 Closeout Ledger

```text
MODULE-W2-LI7-CORRECTION-CLOSEOUT = PASS

Completed internal audits:
1. Li7 triadic-weight proxy pass
2. Two-mode robustness audit
3. Channel-selectivity audit
4. Component ablation audit
5. Real-world-facing observational abundance comparison

Scalar lane status:
OldScalarLi7OverObserved = 3.125337740124628
ScalarLaneStatus = FAILED_FOR_LI7

Triadic-weight lane status:
TriadicLi7OverObservedCenter = 0.9996430364381471
Li7SuppressionFactorVsOldScalar = 3.1264537701984065
PassedSpeciesCount = 4 of 4
TriadicWeightLaneStatus = INTERNAL_PROXY_PASS

Mechanism:
QVDominanceConfirmed = True
CIFOnlyStatus = FAIL_OR_EXPECTED_FAIL
RFLOnlyStatus = FAIL_OR_EXPECTED_FAIL
NoQVRenormalizedStatus = FAIL_OR_EXPECTED_FAIL
ChannelSelectivityConfirmed = True
Be7Li7BranchSelectivityIndex = 3.76031

No-retune conditions:
FrozenModuleGPacketUsed = True
EmpiricalTargetsUsedInPrediction = False
ParameterSearchPerformed = False
MCMCUsed = False
...
Final revised boundary:
InternalW2Status = TRIADIC_WEIGHT_PROXY_PASS
ExternalW2Status = PUBLIC_BBN_NETWORK_VALIDATION_PENDING
AllowedClaim = RFC internal W2 Li7 scalar failure is corrected by triadic channel weights under no-retune proxy conditions.
DisallowedClaim = RFC has completed external public-code resolution of Li7.
```

---

## Supersession Rule for Earlier W2 Entries

Earlier W2 sections that say `Li7 retained as wall`, `Li7 solved = NO`, or `Li7WallConfirmed = True` should be read as the historical status of the **old scalar proxy lane** and the external public-code boundary.

This revision supersedes those lines only in the narrow internal-proxy sense:

```text
Historical scalar-lane conclusion: Li7 wall confirmed.
Revised triadic-weight proxy conclusion: Li7 internal proxy pass.
External public-code conclusion: still pending.
```

The corrected theorem-safe wording is:

```text
RFC does not claim an externally validated Li7 solution here. The revised W2 simulations show that the previous internal Li7 failure was caused by scalar compression of the triadic projection. Restoring separate QV/CIF/RFL channel weights produces a QV-dominant, branch-selective Be7/Li7 suppression while preserving the frozen Module G packet and no-retune guardrails.
```

---

# Original Metadata Trace Preserved Below

The remainder of this file preserves the original raw Wolfram reproducibility trace unchanged, except for the revised header/addendum above.
